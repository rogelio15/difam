...
public class panelCategorias extends javax.swing.JPanel {
    ...
    public void llenarTabla(){
        miTablaCat.setModel(new DefaultTableModel(frmPrincipal.miConex.consDatosParaTablas("*","Categoria", "order by nombrecat"),columnas));
        miTablaCat.getColumnModel().getColumn(0).setPreferredWidth(40);
        miTablaCat.getColumnModel().getColumn(1).setPreferredWidth(950);

        
//        ArrayList misDatos = frmPrincipal.consultas.misCategorias();
//        if(!misDatos.isEmpty()){
//            for(int i=0;i<misDatos.size();i++){
//                miModelo.addRow(new String[2]);
//                HashMap registro = (HashMap)misDatos.get(i);
//                miModelo.setValueAt(registro.get("idCategoria"),i,0);
//                miModelo.setValueAt(registro.get("nombreCat"),i,1);
//            }
//        }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Descripción de Categoria:");
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        if(miTablaCat.getSelectedRow()>=0){
            if((JOptionPane.showConfirmDialog(null, "¿Esta seguro de Eliminar el Registro?","Atencion!!",JOptionPane.YES_NO_OPTION))==JOptionPane.YES_OPTION){
                frmPrincipal.miConex.eliminaRegistro("categoria", "idCategoria", miTablaCat.getModel().getValueAt(miTablaCat.getSelectedRow(),0).toString());
            }
        }else{
        ...
        if(txtNombre.getText().equals("")){
            ...
            if(AoC){
                if((JOptionPane.showConfirmDialog(null, "¿Esta seguro de el Cambio?","Atencion!!",JOptionPane.YES_NO_OPTION))==JOptionPane.YES_OPTION){
                    frmPrincipal.miConex.actualizaRegistro("categoria", "nombreCat='"+txtNombre.getText().toUpperCase()+"'", "idCategoria='"+miTablaCat.getModel().getValueAt(miTablaCat.getSelectedRow(),0).toString()+"'");
                }else{
                    txtNombre.setText("");
                }
            }else{
                frmPrincipal.miConex.altaRegistro("categoria","null,'" + txtNombre.getText() + "'");
            }
            AoC=false;
        }
        ControlBotones(false,true);
